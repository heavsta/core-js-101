...
function timeSpanToString(startDate, endDate) {
  return new Date(new Date(endDate) - new Date(startDate))
    .toISOString()
    .slice(11, -1);
}
...
function angleBetweenClockHands(date) {
  const hours = +date.toISOString().slice(11, 13) % 12;
  const minutes = +date.toISOString().slice(14, 16);
  const minuteAngle = minutes * 6;
  const hourAngle = hours * 30 + minutes * 0.5;
  const angle = Math.abs(hourAngle - minuteAngle);
  const minAngle = Math.min(angle, 360 - angle);
  return (minAngle * Math.PI) / 180.0;
}
